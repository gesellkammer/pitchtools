...
class NoteParts:
    ...
    __slots__ = ('octave', 'diatonic_name', 'alteration', 'cents_deviation')
    ...
    def midinote(self) -> float:
        pc = _notes2[self.diatonic_name]
        alt = self.alteration
        if alt == '#':
            pc += 1
        elif alt == "b":
            pc -= 1
        return (self.octave + 1) * 12 + pc + self.cents_deviation/100.
...
class PitchConverter:
    ...
    Args:
        ...
        unicode_accidentals: use unicode symbols for accidentals
    ...
    __slots__ = ('a4', 'eighthnote_symbol', 'unicode_accidentals')

    def __init__(self, a4=442.0, eightnote_symbol=True, unicode_accidentals=False):
        self.a4 = a4
        self.eighthnote_symbol = eightnote_symbol
        self.unicode_accidentals = unicode_accidentals
    ...
    def midi_to_note_parts(self, midinote: float) -> tuple[int, str, str, int]:
        ...
        sharps = _sharps if not self.unicode_accidentals else _unicode_sharps
        flats = _flats if not self.unicode_accidentals else _unicode_flats
        if cents == 0:
            return (octave, sharps[ps], "", 0)
        elif cents == 50:
            if ps in {1, 3, 6, 8, 10}:
                return (octave, sharps[ps + 1], "-", 0)
            return (octave, sharps[ps], "+", 0)
        elif cents == 25 and self.eighthnote_symbol:
            symbol = ">" if not self.unicode_accidentals else "↑"
            if ps in (6, 10,):
                return (octave, flats[ps], symbol, 0)
            return (octave, sharps[ps], symbol, 0)
            ...
            if ps > 11:
                octave += 1
            return (octave, flats[ps], "", -cents)
        else:
            return (octave, sharps[ps], "", cents)
...
def n2m(note: str) -> float:
    ...
    * 2nd format (octave first): ``2C#``, ``4D+``, ``7Eb-14``
    ...
    """
    oct, pch, alt, cents = _split_notename(note)
    pc = _notes2[pch]
    if alt == '#' or alt == '♯':
        pc += 1
    elif alt == "b" or alt == '♭':
        pc -= 1
    return (oct + 1) * 12 + pc + cents/100.


# @_cache
def _n2m(note: str) -> float:
    """
    Convert a notename to a midinote
    ...
    * 2nd format (octave first): ``2C#``, ``4D+``, ``7Eb-14``
    ...
    if len(pitchstr) == 2:
        ...
        if alt == "#":
            ...
            raise ValueError(f"Could not parse alteration in {note}")
    ...
    if not microstr:
        ...
        raise ValueError(f"Could not parse notename's suffix '{microstr}' of note '{note}'")
...
def quantize_notename(notename: str, divisions_per_semitone: int) -> str:
    """
    ...
    cents = int(round(parts.cents_deviation / 100 * divisions_per_semitone) / divisions_per_semitone * 100)
    ...
    return construct_notename(parts.octave, parts.diatonic_name, parts.alteration, cents)
...
def _split_notename_regex(notename: str) -> NoteParts:
    # This is somewhat slower (~ 50% slower) than the version below
    if m := _re.match(_r2b, notename):
        octave, pitch, centstr = m.groups()
    elif m := _re.match(_r1b, notename):
        pitch, octave, centstr = m.groups()
    else:
        raise ValueError(f"Invalid notename '{notename}'")
    if pitch[-1] in 'b#':
        letter, alter = pitch[:-1], pitch[-1]
    else:
        letter, alter = pitch, ''
...
def _split_notename(notename: str) -> tuple[int, str, str, int]:
    ...
    if c0.isdecimal() or c0 == '-':
        ...
        if c0 == '-':
            ...
            cursor = 0
            octavesign = 1
        # common case: octave between 0 and 9
        n0 = notename[cursor]
        n1 = notename[cursor+1]
        if n0.isdecimal() and not n1.isdecimal():
            octave, letter = int(n0), n1
            cursor += 2
        else:
            for i in range(10):
                if not notename[cursor+i].isdecimal():
                    break
            else:
                raise ValueError(f"Invalid octave in '{notename}'")
            octave = int(notename[cursor:cursor+i])
            cursor += i
            letter = notename[cursor]
            cursor += 1
        octave *= octavesign
        if len(notename) == cursor:
            cents = 0
            alter = ""
        else:
            r0 = notename[cursor]
            if r0 == 'b' or r0 == '#':
                alter = r0
                centstr = notename[cursor+1:]
            else:
                centstr = notename[cursor:]
                alter = ''
            cents = _parse_centstr(centstr) if centstr else 0
            if cents is None:
...
def transpose(notename: str, interval: float, white_enharmonic=True) -> str:
    ...
    chromatic2 = _chromatic_transpositions[chromatic1][rounded_interval % 12]
